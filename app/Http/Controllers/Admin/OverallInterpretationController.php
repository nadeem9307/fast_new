...
use App\Model\OverAllInterpretation;
...
class OverallInterpretationController extends Controller {
    ...
    public function index() {
        try {
//            $overall_range = '';
//            $data = OverAllInterpretation::GetInterpretationRange($overall_range);
//            $combos = $this->generate_combinations($data);
            $levels = Levels::All();
            return view('admin.overall_interpretation.set_interpretation',  compact('levels'));
    ...
    public function OverAllInterpretation(request $request) {
        if ($request->ajax()) {
            ...
            if ($validator->fails()) {
                ...
                try {
                    $interpretation = OverAllInterpretation::SaveOverAllInterpretation($request);
    ...
    public function getOverallRange(Request $request,$level_id) {
        try {
...
//                );
            ...
            if ($range) {
                $data = OverAllInterpretation::GetInterpretationRange($range->id,$level_id);
                ...
                $combos = array_filter($comboss, function($v, $k) use($i, $range) {//$cats,$interpretations
...
//                        $interp->category_ids = implode(",", array_column($cats, 'category_id'));
...
//                    }
                ...
                return view('admin.overall_interpretation.overall_interpretation', compact('combos', 'cats', 'overall_ids', 'range_id'));
            } else {
                abort(404);
            }
        } catch (\Illuminate\Database\QueryException $ex) {
    ...
    public function edit(Request $request) {
        if ($request->ajax()) {
            try {
                $status = "success";
                $overall = OverAllInterpretation::EditOverallInterpretation($request);
                if ($overall !== false) {
