...
use App\Model\ParentInterpretation;
use Response;
use Validator;
use DB;
...
class InterpretationController extends Controller
{
    ...
    {
        $category_id = $id;
        $datas = ParentInterpretation::GetInterpretationRange($id);
        $data = $datas['data'];
        $cat = $datas['cat'];
        $cate_data = $datas['cate_data'];
        return view('admin.parent.interpretation.interpretation_by_category', compact('data', 'category_id', 'cate_data', 'cat'));
    }
    ...
    public function store(request $request)
    {
        if ($request->ajax()) {
            ...
            if ($validator->fails()) {
                ...
                $interpretation = ParentInterpretation::SaveInterpretation($request);
    ...
    public function show(Request $request, $id)
    {
        try {
            ...
            $category = ParentInterpretation::getInterByCategory($perpage, $page, $offset, $draw, $cat_id);
    ...
    public function edit(Request $request)
    {
        if ($request->ajax()) {
            try {
                $status = "success";
                $interpretation = ParentInterpretation::EditInterpretation($request);
    ...
    public function deleteInterpretation(Request $request)
    {
        if ($request->ajax()) {
            try {
                if ($request->id) {
                    $interpretation = ParentInterpretation::find($request->id);
        ...
        if ($request->ajax()) {
            try {
                ...
                if ($validator->fails()) {
                    ...
                    $interpretation = ParentInterpretation::SaveCatInterpretation($request);
                    if ($interpretation['status'] == 'error') {
                        return (json_encode(array('status' => 'error', 'message' => "Interpretation already exist.")));
                    } else {
                        try {
