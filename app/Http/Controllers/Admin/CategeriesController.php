...
class CategeriesController extends Controller
{
    ...
    public function index()
    {
        try {
            ...
            return view('admin.categories.category', compact('levels'));
        ...
        if ($request->ajax()) {
            try {
                ...
                if ($validator->fails()) {
                    ...
                    if (isset($failedRules['levels']))
                        $errorMsg = $failedRules['levels'][0] . "\n";
    ...
    public function show(Request $request)
    {
        try {
            ...
            $category = Category::getCategory($perpage, $page, $offset, $draw, $cate);
    ...
    public function destroy(Request $request)
    {
        try {
            if ($request->ajax()) {
                $cat = Category::deleteCategory($request);
                if ($cat == 'success') {
    ...
    public function CategoryStatus(Request $request)
    {
        try {
            if ($request->ajax()) {
                $cat = Category::ActivateDeactivateStatus($request);
                if ($cat == 'success') {
