...
class Interpretation extends Model {
    ...
    public $table = "category_wise_interpretation";
...
//                        })
    ...
    public static function EditInterpretation($request) {
        $id = $request->range_id;
        if ($id != '') {
            return Interpretation::where("range_id", $request->range_id)->where('category_id', $request->category_id)->select('*')->first();
        } else {
            return false;
        }
    }
    ...
    public static function GetInterpretationRange($category_id) {
        try {
            ...
            if ($cat) {
                ...
                $cate_data = Interpretation::select('range_id')->where('category_id', $category_id)->get()->toArray();
    ...
    public static function SaveCatInterpretation($request) {
        ...
        $category_id = $request->category_id;
        /* --------------------- check if Interpretation already exist ------------------- */
        $check = Interpretation::where('range_id', '=', $request->input('range_id'))
