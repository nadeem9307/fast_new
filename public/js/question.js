$(document).ready(function() {
    ...
    },
    {
      ...
      template: function (row) {
        ...
        {
            return '\
            <div><audio controls class="player" src="./storage/app/questions/'+row.file_attached+'"></audio></div>\
            ';
        }
        else if(file_ext[1]=='mp4')
        {
            ...
            <div><video width="100" id="video">\
              <source src="./storage/app/questions/'+row.file_attached+'" type="video/mp4">\
            </video></div>\
                        ';
        }
        ...
        {
          return '\
          <div><img src="./storage/app/questions/'+row.file_attached+'" width="100" height="100"></div>\
          ';
        }
        else
        {
...
//      var dropup = (row.getDatatable().getPageSize() - row.getIndex()) <= 4 ? 'dropup' : '';
    ...
    $('#categorylist').on('change',function(){
      ...
      var sublid = $('#sublevellist').val();
//      var semid = $('#semesterllist').val();
//      datatable.setDataSourceQuery({category_id: value, country_id: coutid, level_id: levelid, sublevel_id: sublid, semester_id:semid});
      datatable.setDataSourceQuery({category_id: value, country_id: coutid, level_id: levelid, sublevel_id: sublid});
      datatable.reload();
    });
    $('#countrylist').on('change',function(){
      ...
      var sublid = $('#sublevellist').val();
//      var semid = $('#semesterllist').val();
      datatable.setDataSourceQuery({category_id: catid, country_id: value, level_id: levelid, sublevel_id: sublid});
      datatable.reload();
    });
    $('#levellist').on('change',function(){
      var value = $(this).val();
      var catid = $('#categorylist').val();
      var countyid = $('#countrylist').val();
      var sublid = $('#sublevellist').val();
//      var semid = $('#semesterllist').val();
      datatable.setDataSourceQuery({category_id: catid, country_id: countyid, level_id: value, sublevel_id: sublid});
      datatable.reload();
    });
    $('#sublevellist').on('change',function(){
      var value = $(this).val();
      var catid = $('#categorylist').val();
      var countyid = $('#countrylist').val();
      var levelid = $('#levellist').val();
      var semid = $('#semesterllist').val();
      datatable.setDataSourceQuery({category_id: catid, country_id: countyid, level_id:levelid, sublevel_id: value});
      datatable.reload();
    });
    $('#semesterllist').on('change',function(){
      var value = $(this).val();
      var catid = $('#categorylist').val();
      var countyid = $('#countrylist').val();
      var levelid = $('#levellist').val();
      var sublid = $('#sublevellist').val();
      datatable.setDataSourceQuery({category_id: catid, country_id: countyid, level_id:levelid, sublevel_id: sublid, semester_id:value});
      datatable.reload();
    });
