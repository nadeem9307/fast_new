        ...
        <div class="mr-auto">
            ...
            <ul class="m-subheader__breadcrumbs m-nav m-nav--inline">
                ...
                <li class="m-nav__item">
                    <a href="{{ route('category')}}" class="m-nav__link">
...
<!--                            <div class="col-md-4">
                                <div class="m-input-icon m-input-icon--left">
                                    <div class="m-form__control">
                                        @if(!empty($levels))
                                        <select class="form-control m-input" id="level_status" name="level_status">
                                    ...
                                    <div class="m-dropdown__body">
                                        <div class="m-dropdown__content">
                                            <ul class="m-nav">
                                                ...
                                                <li class="m-nav__item">
                                                    <a href="#" class=" m-nav__link" id="addcategoryBtn" data-toggle="modal" data-target="#model_add_category">
                                                        <i class="m-nav__link-icon flaticon-share"></i>
                                                        <span class="m-nav__link-text"> Add Category</span>
                                                    </a>
                                                </li>
                                                <li class="m-nav__item">
                                                    <a href="{{ route('overallRange')}}" class="m-nav__link">
    ...
    <div class="modal fade" id="model_add_category" tabindex="-1" role="dialog" aria-labelledby="" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <form class="m-form m-form--fit m-form--label-align-right" name="add_category" id="add_category" method="POST" action="{{ route('addcategory')}}">
                    {{ csrf_field() }}
                    <div class="modal-body">
                        ...
                        <div class="form-group m-form__group row m--margin-top-20">
                            ...
                            <div class="col-lg-9 col-md-9 col-sm-12">
                                <div class="m-typeahead">
                                    @if(!empty($levels))
                                    <select class="form-control m-input" id="levels" name="levels">
                                        <option value="">Please select level</option>
                                        @foreach($levels as $level)
                                        <option value="{{$level['id']}}">{{$level['level_name']}}</option>
                                        @endforeach
...
//                },
                ...
                },
                {
                    ...
                    template: function (row) {
                        return '<a href="' + APP_URL + '/interpretation/id/' + row.id + '" class="btn btn-success background_gradient btn-view" title="Manage">\
                   Manage\
                   </a>\
                 ';
                    },
        ...
        /*----------------------level  wise  filter data  ----------------------------*/
        $('#level_status').on('change', function () {
            var value = $(this).val();
            if (value != '')
            {
                datatable.setDataSourceQuery({level_id: value});
                datatable.reload();
            } else
            {
                datatable.setDataSourceQuery({level_id: ''});
                datatable.reload();
            }
    ...
    $('#addcategoryBtn').click(function (e) {
        $('#model_add_category').find('.modal-title').html('Add Category');
        $('#category_id').val('');
        $('#levels').val('');
        $('#category').val('');
    });
    ...
    $('#add_cate').click(function (e) {
        ...
        if (levels == '') {
            swal("Error", "Levels Name  is required", "error");
            return false;
        }
    ...
    function activate_deactivateCategory(id) {
        var path = APP_URL + "/category/status";
    ...
    function getCategoryDetail(id) {
        var path = APP_URL + "/category/edit";
        $.ajax({
            method: "GET",
            url: path,
            data: {
            ...
            success: function (result) {
                ...
                if (res.status == 'error') {
                    ...
                    var levels = $('#levels').val(data.level_id);
    ...
    function deleteCategory(id) {
        var path = APP_URL + "/category/destroy";
        var _this = $(this);
        swal({
