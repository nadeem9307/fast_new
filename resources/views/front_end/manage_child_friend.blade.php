        ...
        <div class="modal-content">
            ...
            <form name="registerchild" id="registerchild" action="{{route('saveChild')}}" method="POST">
                <div class="modal-body">
                    ...
                    <div class="form-group m-form__group">
                        <input name="age" class="form-control m-input" id="age" type="text" placeholder="Enter age in years" value="">
                    </div>
                    <div class="form-group m-form__group">
...
//        var userpassword = $('#userpassword').val();
    ...
    if (age == '') {
    swal("Error", "Age is required", "error");
    return false;
    }
    $.ajax({
    method: 'POST',
...
//                           
            ...
            width: 110,
                    ...
                    if (row.from_status == '1' && row.request_status == '2')
                    {
							...
							let current_user = (row.to_id!=user_id ? row.to_id: row.from_id);
                    return '\
                                <a href="' + view_path + current_user + '" class="theme-text" title="view child">\
                    ...
                    if (row.from_status == '2' && row.request_status == '2')
                    {
                            @if ($type == '1' && $user-> user_type == '2' || $type == '2' && $user-> user_type == '3')
								let current_user = (row.to_id!=user_id ? row.to_id: row.from_id);
                                return '\  <a href="' + view_path + current_user + '" class="theme-text" title="view child">\
                               <i class="flaticon-visible"></i> &nbsp;\
                               </a>\
                                <a href="javascript:;" class="theme-text" title="Untag">\
                                <i class="fa fa-user-times" onclick=untagUser(' + row.id + ',"' + row.to_name + '")></i> \
